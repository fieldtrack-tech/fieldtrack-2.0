...
export const apiKeysRepository = {
  async create(
    ...
    const { data, error } = await supabase
      ...
      .select("id, organization_id, name, key_hash, key_prefix, scopes, active, request_count, error_count, created_at, updated_at, last_used_at")
      .single();
  ...
  async findByHash(keyHash: string): Promise<ApiKeyAuthRecord | null> {
    const { data, error } = await supabase
      .from("api_keys")
      .select("id, organization_id, key_hash, scopes, active")
      .eq("key_hash", keyHash)
      .eq("active", true)
      .is("revoked_at", null)
      .limit(1)
    ...
    return (data as ApiKeyAuthRecord | null) ?? null;
  },
